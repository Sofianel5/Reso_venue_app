...
class AddTimeSlotBloc extends Bloc<AddTimeSlotEvent, AddTimeSlotState> {
  ...
  Stream<AddTimeSlotState> mapEventToState(AddTimeSlotEvent event) async* {
    if (event is AddTimeSlotAttempt) {
      ...
      print(event.start);
      print(event.stop);
      final result = await addTimeSlot(AddTimeSlotParams(venue: user.venues[user.currentVenue], end: event.stop, start: event.start, numAttendees: event.numAttendees, type: event.type));
